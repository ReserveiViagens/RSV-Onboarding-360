 ...
 */
router.get('/customers', authorize(['admin', 'manager']), asyncHandler(async (req, res) => {
  // Customer overview
 ...
 */
router.get('/packages', authorize(['admin', 'manager']), asyncHandler(async (req, res) => {
  // Package overview
  ...
  const popularPackages = await db('travel_packages')
    .select('id', 'name', 'destination', 'total_bookings', 'average_rating', 'base_price')
    .where('status', 'active')
  ...
  switch (type) {
    ...
    case 'bookings':
      data = await db('bookings')
        .select('booking_number', 'title', 'status', 'total_amount', 'start_date', 'end_date', 'created_at')
        .where('created_at', '>=', start_date)
        .where('created_at', '<=', end_date)
        .orderBy('created_at', 'desc');
    ...
    case 'customers':
      data = await db('customers')
        .select('name', 'email', 'status', 'vip_level', 'total_spent', 'total_bookings', 'created_at')
        .where('created_at', '>=', start_date)
        .where('created_at', '<=', end_date)
        .orderBy('created_at', 'desc');
    ...
    case 'packages':
      data = await db('travel_packages')
        .select('name', 'destination', 'base_price', 'status', 'total_bookings', 'average_rating', 'created_at')
        .orderBy('total_bookings', 'desc');
