...
router.post('/login', [
  ...
  const user = await db('users')
    .where({ email })
    .select('id', 'name', 'email', 'password_hash', 'role', 'status', 'login_attempts', 'locked_until')
    .first();
  ...
  if (user.status !== 'active') {
    logger.logSecurity('Login attempt with inactive user', {
      userId: user.id,
      email,
      status: user.status,
      ip: req.ip
  ...
  if (!isValidPassword) {
    ...
    if (newAttempts >= maxAttempts) {
      ...
      logger.logSecurity('Account locked due to failed login attempts', {
        userId: user.id,
        email,
...
router.get('/me', require('../middleware/auth').authenticateToken, asyncHandler(async (req, res) => {
  const user = await db('users')
    ...
    .select('id', 'name', 'email', 'role', 'status', 'department', 'position', 'phone', 'avatar_url', 'preferences', 'created_at', 'last_login')
    .first();
...
router.post('/forgot-password', [
  ...
  if (!user) {
    logger.logSecurity('Password reset attempt for non-existent email', { email, ip: req.ip });
    return res.json({
      success: true,
...
router.post('/2fa/setup', authenticateToken, asyncHandler(async (req, res) => {
  ...
  const { secret, manualEntryKey, qrCodeUrl } = generateTwoFactorSecret(req.user.email);
...
router.post('/2fa/verify', [
  ...
  if (!token && !backupCode) {
    throw new AppError('Token or backup code required', 400, 'TOKEN_OR_BACKUP_CODE_REQUIRED');
  }
  ...
  if (!user) {
    throw new AppError('User not found or 2FA not enabled', 404, 'USER_NOT_FOUND');
  }
  ...
  if (!isValid) {
    logger.logSecurity('Invalid 2FA token/backup code attempt', {
      userId: user.id,
      email,
      hasToken: !!token,
    ...
    throw new AppError('Invalid token or backup code', 401, 'INVALID_TWO_FA_TOKEN');
  }
...
router.post('/2fa/disable', [
  ...
  if (backupCode) {
    ...
    throw new AppError('2FA token or backup code required', 400, 'TWO_FA_VERIFICATION_REQUIRED');
  }

  if (!isValid2FA) {
    throw new AppError('Invalid 2FA token or backup code', 401, 'INVALID_TWO_FA_TOKEN');
  }
