...
router.get('/', [
  ...
  query('type').optional().isIn(['hotel', 'flight', 'car', 'tour', 'package', 'activity']),
...
router.post('/', [
  ...
  body('type').isIn(['hotel', 'flight', 'car', 'tour', 'package', 'activity']),
  ...
  if (targetUserId !== req.user.id && !['admin', 'manager'].includes(req.user.role)) {
    throw new AppError('Cannot create booking for other users', 403, 'INSUFFICIENT_PERMISSIONS');
  }
  ...
  const bookingData = {
    ...
    guest_details: req.body.guest_details ? JSON.stringify(req.body.guest_details) : null,
    special_requests: req.body.special_requests ? JSON.stringify(req.body.special_requests) : null,
  ...
  logger.info(`Booking created: ${bookingNumber} by ${req.user.email} for user ${fullBooking.user_email}`);
...
router.put('/:id', [
  ...
  body('description').optional().trim().isLength({ max: 1000 }),
  body('status').optional().isIn(['pending', 'confirmed', 'in_progress', 'completed', 'cancelled', 'refunded']),
  ...
  if (req.user.role === 'user' && existingBooking.user_id !== req.user.id) {
    throw new AppError('Cannot update other users bookings', 403, 'INSUFFICIENT_PERMISSIONS');
  }
  ...
  if (updateData.status === 'completed' && existingBooking.payment_status !== 'paid') {
    throw new AppError('Cannot complete booking with pending payment', 400, 'PAYMENT_REQUIRED');
  }

  // Update pending amount if total amount changes
  if (updateData.total_amount) {
    updateData.pending_amount = parseFloat(updateData.total_amount) - parseFloat(existingBooking.paid_amount);
  }
  ...
  logger.info(`Booking updated: ${existingBooking.booking_number} by ${req.user.email}`);
...
router.post('/:id/cancel', [
  ...
  if (req.user.role === 'user' && booking.user_id !== req.user.id) {
    throw new AppError('Cannot cancel other users bookings', 403, 'INSUFFICIENT_PERMISSIONS');
  }

  // Check if booking can be cancelled
  if (['cancelled', 'refunded', 'completed'].includes(booking.status)) {
    throw new AppError('Booking cannot be cancelled in current status', 400, 'INVALID_STATUS_CHANGE');
  }
  ...
  logger.info(`Booking cancelled: ${booking.booking_number} by ${req.user.email}`);
...
router.delete('/:id', [
  ...
  logger.info(`Booking deleted: ${booking.booking_number} by ${req.user.email}`);
...
router.get('/stats', authorize(['admin', 'manager']), asyncHandler(async (req, res) => {
  const stats = await db('bookings')
    .select(
      ...
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as pending_bookings', ['pending']),
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as confirmed_bookings', ['confirmed']),
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as completed_bookings', ['completed']),
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as cancelled_bookings', ['cancelled']),
