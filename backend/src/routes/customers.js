...
router.get('/', [
  ...
  query('vip_level').optional().isIn(['none', 'bronze', 'silver', 'gold', 'platinum']),
  ...
  let query = db('customers').select(
    'id', 'name', 'email', 'phone', 'document', 'city', 'state',
    'status', 'type', 'vip_level', 'total_bookings', 'total_spent',
...
router.get('/:id', [
  ...
  const bookings = await db('bookings')
    ...
    .select('id', 'booking_number', 'title', 'status', 'total_amount', 'start_date', 'end_date', 'created_at')
    .orderBy('created_at', 'desc')
    .limit(10);
  ...
  const payments = await db('payments')
    .where({ user_id: customer.id })
    .select('id', 'transaction_id', 'amount', 'status', 'method', 'created_at')
    .orderBy('created_at', 'desc')
    .limit(10);
...
router.post('/', [
  ...
  const existingCustomer = await db('customers').where({ email: customerData.email }).first();
  if (existingCustomer) {
    throw new AppError('Customer with this email already exists', 400, 'EMAIL_EXISTS');
  }
...
router.put('/:id', [
  ...
  body('vip_level').optional().isIn(['none', 'bronze', 'silver', 'gold', 'platinum']),
  body('notes').optional().trim(),
  body('preferred_contact').optional().isIn(['email', 'phone', 'whatsapp']),
  body('marketing_consent').optional().isBoolean(),
], asyncHandler(async (req, res) => {
  ...
  if (updateData.email && updateData.email !== existingCustomer.email) {
    const emailExists = await db('customers').where({ email: updateData.email }).first();
    if (emailExists) {
  ...
  logger.info(`Customer updated: ${existingCustomer.email} by ${req.user.email}`);
...
router.delete('/:id', [
  ...
  const bookingCount = await db('bookings').where({ user_id: id }).count('* as count').first();
  if (bookingCount.count > 0) {
    ...
    logger.info(`Customer deactivated: ${customer.email} by ${req.user.email}`);
...
router.get('/stats', authorize(['admin', 'manager']), asyncHandler(async (req, res) => {
  const stats = await db('customers')
    .select(
      ...
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as active_customers', ['active']),
      db.raw('COUNT(CASE WHEN type = ? THEN 1 END) as individual_customers', ['individual']),
      db.raw('COUNT(CASE WHEN type = ? THEN 1 END) as corporate_customers', ['corporate']),
      db.raw('COUNT(CASE WHEN vip_level != ? THEN 1 END) as vip_customers', ['none']),
  ...
  const topCustomers = await db('customers')
    .select('id', 'name', 'email', 'total_spent', 'total_bookings', 'vip_level')
    .where('status', 'active')
