...
router.get('/', [
  ...
  query('category').optional().isIn(['package', 'hotel', 'flight', 'tour', 'activity']),
  query('status').optional().isIn(['active', 'inactive', 'draft', 'sold_out']),
  ...
  let query = db('travel_packages').select(
    'id', 'name', 'slug', 'short_description', 'destination', 'category',
    'duration_days', 'duration_nights', 'base_price', 'adult_price', 
    'child_price', 'currency', 'min_people', 'max_people',
    'status', 'is_featured', 'hotel_name', 'hotel_stars',
    'average_rating', 'total_reviews', 'total_bookings', 'created_at'
  );
...
router.get('/:id', [
  ...
  const package = await db('travel_packages').where({ id }).first();
  ...
  const packageData = {
    ...package,
    included_services: package.included_services ? JSON.parse(package.included_services) : [],
    excluded_services: package.excluded_services ? JSON.parse(package.excluded_services) : [],
    itinerary: package.itinerary ? JSON.parse(package.itinerary) : {},
    requirements: package.requirements ? JSON.parse(package.requirements) : [],
    recommendations: package.recommendations ? JSON.parse(package.recommendations) : [],
    hotel_amenities: package.hotel_amenities ? JSON.parse(package.hotel_amenities) : [],
    seasonal_pricing: package.seasonal_pricing ? JSON.parse(package.seasonal_pricing) : {},
    availability_calendar: package.availability_calendar ? JSON.parse(package.availability_calendar) : {},
    gallery_images: package.gallery_images ? JSON.parse(package.gallery_images) : [],
    gallery_videos: package.gallery_videos ? JSON.parse(package.gallery_videos) : [],
    tags: package.tags ? JSON.parse(package.tags) : []
  };

  // Get related bookings count for this package
  const bookingStats = await db('bookings')
    .where('title', 'like', `%${package.name}%`)
    .select(
      db.raw('COUNT(*) as total_bookings'),
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as confirmed_bookings', ['confirmed']),
...
router.post('/', [
  ...
  body('category').optional().isIn(['package', 'hotel', 'flight', 'tour', 'activity']),
  ...
  if (existingPackage) {
    throw new AppError('Package with similar name already exists', 400, 'SLUG_EXISTS');
  }
  ...
  ['included_services', 'excluded_services', 'itinerary', 'requirements', 
   'recommendations', 'hotel_amenities', 'seasonal_pricing', 'availability_calendar',
   'gallery_images', 'gallery_videos', 'tags'].forEach(field => {
    if (packageData[field] && typeof packageData[field] === 'object') {
      packageData[field] = JSON.stringify(packageData[field]);
    }
  });

  const [newPackage] = await db('travel_packages').insert(packageData).returning('*');
  ...
  logger.info(`Travel package created: ${newPackage.name} by ${req.user.email}`);
...
router.put('/:id', [
  ...
  logger.info(`Travel package updated: ${existingPackage.name} by ${req.user.email}`);
...
router.delete('/:id', [
  ...
  if (bookingCount.count > 0) {
    ...
    await auditLogger.log(
      req.user.id,
      'package_deactivated',
      `Travel package deactivated: ${package.name}`,
    ...
    await auditLogger.log(
      req.user.id,
      'package_deleted',
      `Travel package deleted: ${package.name}`,
      { package_id: id }
    );
...
router.get('/stats', authorize(['admin', 'manager']), asyncHandler(async (req, res) => {
  const stats = await db('travel_packages')
    .select(
      ...
      db.raw('COUNT(CASE WHEN status = ? THEN 1 END) as active_packages', ['active']),
      db.raw('COUNT(CASE WHEN is_featured = true THEN 1 END) as featured_packages'),
  ...
  const topRated = await db('travel_packages')
    .select('id', 'name', 'destination', 'base_price', 'average_rating', 'total_reviews')
    .where('status', 'active')
...
router.get('/featured', asyncHandler(async (req, res) => {
  const featuredPackages = await db('travel_packages')
    .select(
      ...
      'duration_days', 'base_price', 'currency', 'hotel_name', 
      'hotel_stars', 'average_rating', 'total_reviews'
    )
    .where('status', 'active')
