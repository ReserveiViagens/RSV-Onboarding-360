...
router.post('/:type', [
  ...
  logger.info(`Files uploaded: ${files.length} files of type ${type} by ${req.user.email}`);
...
router.delete('/:id', [
  ...
  try {
    ...
    if (file.uploaded_by !== req.user.id && !['admin', 'manager'].includes(req.user.role)) {
      throw new AppError('Cannot delete other users files', 403, 'INSUFFICIENT_PERMISSIONS');
    }
...
router.post('/validate', [
  ...
  if (size > maxSize) {
    ...
    validation.errors.push(`File size ${(size / 1024 / 1024).toFixed(2)}MB exceeds maximum ${(maxSize / 1024 / 1024).toFixed(2)}MB`);
  }
  ...
  if (!allowed.includes(ext)) {
    validation.valid = false;
    validation.errors.push(`File extension ${ext} not allowed for type ${type}. Allowed: ${allowed.join(', ')}`);
  }
