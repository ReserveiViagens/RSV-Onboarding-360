...
let workflows = [
  {
    ...
    description: 'Workflow para aprovação automática de reservas acima de R$ 2000',
    status: 'active',
    type: 'approval',
  ...
  },
  {
    ...
    conditions: [
      { field: 'checkout_date', operator: 'passed_days', value: 3 }
    ],
    actions: [
...
router.put('/:id', [
  ...
  logger.info(`Workflow updated: ${updatedWorkflow.name} by ${req.user.email}`);
...
router.get('/stats', authorize(['admin', 'manager']), asyncHandler(async (req, res) => {
  const stats = {
    ...
    average_success_rate: workflows.reduce((sum, w) => sum + w.success_rate, 0) / workflows.length,
    by_type: {
      approval: workflows.filter(w => w.type === 'approval').length,
      notification: workflows.filter(w => w.type === 'notification').length,
      automation: workflows.filter(w => w.type === 'automation').length
